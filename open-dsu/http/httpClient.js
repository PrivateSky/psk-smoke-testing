...
assert.callback('HTTP test', (callback) => {
    tir.launchVirtualMQNode((err, port) => {
        ...
        http.doPut(`http://localhost:${port}/bricks/put-brick`, { test: 'da' }, (err, response) => {
            ...
            http.fetch(`http://localhost:${port}/bricks/get-brick/${brickHash}`).then((responseGetBrick) => {
                ...
                http.fetch(`http://localhost:${port}/bricks/downloadMultipleBricks?hashes=${brickHash}`).then((responseMultiple) => {
